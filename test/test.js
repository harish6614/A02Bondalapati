...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
    assert.equal(calculateCost(5, 5), 25, 'Tested with two relatively small positive numbers');
    assert.equal(calculateCost(-5, -5), 25, 'Tested with two negative numbers.');
    assert.equal(calculateCost(500, 500), 250000, 'Tested with two large positive numbers.');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.throws(function () { calculateCost(null); }, /One or more given arguments is not a number/, 'Passing in null correctly raises an Error');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.throws(function () { calculateCost("Christine","Christine"); }, /One or more given arguments is not a number/, 'Passing in a string correctly raises an Error');
});
